...
select md5(json_build_object(
  ...
  'eventOps', (select coalesce(json_agg(t order by user_id,operation_key), '[]'::json) from loopedin_private.loopedin_event_create_operations t),
  'messageOps', (select coalesce(json_agg(t order by user_id,operation_key), '[]'::json) from loopedin_private.loopedin_message_create_operations t),
