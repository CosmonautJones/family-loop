...
import { verifyEnvelope } from './build-netlify-deployment-envelope.mjs';
...
try {
  if (!args.get('--envelope') || !args.get('--expected-artifact-sha256') || !args.get('--expected-source-commit') || args.size !== 3) {
    throw new Error('Usage: node scripts/verify-netlify-deployment-envelope.mjs --envelope <path> --expected-artifact-sha256 <sha256> --expected-source-commit <sha>');
  }
