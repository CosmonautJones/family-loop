...
if (-not $AcknowledgeStagingOnly) {
  throw 'Refusing hosted lifecycle QA without -AcknowledgeStagingOnly.'
}
...
try {
  ...
  & node tests/supabase-hosted-account-lifecycle-e2e.mjs
  if ($LASTEXITCODE -ne 0) { throw 'Hosted staging account lifecycle QA failed; provider detail was redacted.' }
} finally {
  $keys = $null
  $publishable = $null
  $secret = $null
