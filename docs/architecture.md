...
The checked-in remote media policies are not yet aligned: `loopedin_event_media` metadata deletion is uploader-or-manager scoped, but the `storage.objects` delete policy permits any event member. Supabase media upload/metadata creation and metadata deletion/object removal are also separate nontransactional calls, so a second-step failure can orphan an object or metadata. These are production blockers for a future authorized remote-storage wave, not failures hidden by the accepted local gate.
