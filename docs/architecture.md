...
The Supabase client uses AsyncStorage for auth-session persistence. Local browser durability is not remote persistence, multi-user synchronization, authenticated authorization, deployed database/RLS, or private-storage proof. Local Docker Supabase now proves migration execution and the documented synthetic media RLS/Storage matrix, but no remote deployment has been verified; hosted synchronization, policy behavior, and operations remain unproven.
