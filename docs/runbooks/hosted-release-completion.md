...
- GitHub Actions run `29448180907` is green for exact source `e52be53165a584b28185015c70c8f8c7ba8e4d0d`. Its downloaded artifact identifies release `0.1.0-e52be53165a5` and internal artifact digest `6f0f57670eca8621178293d80396a8880f66b69a132600aa03f985c685729f31`.
- Netlify deploy `6a57f034c398b8ce7dbe3fc8` contains that fixed artifact and was restored unchanged to the dedicated site's staging alias at `https://loopedin-family.netlify.app` at `2026-07-15T20:41:45.508Z`. Its immutable URL is `https://6a57f034c398b8ce7dbe3fc8--loopedin-family.netlify.app`; no rebuild, Git link, custom domain, Function, or Edge Function was introduced. Prior deploy `6a56f0408614647fe35cf968` remains the exact non-database rollback target.
- The published alias rendered the signed-out configured app over HTTPS with `loopedin-staging · Connected`, release `0.1.0-e52be53165a5`, the expected security/CSP headers, `no-store` runtime configuration, immutable hashed assets, and a missing-asset `404`. Supabase Auth `site_url` is now exactly the staging alias, and the exact allow-list contains the staging alias, both immutable deploy URLs, and future `https://app.travisjohnjones.com`; the prior localhost Site URL and empty allow-list are superseded.
...
- Custom SMTP remains absent, so recovery proves only staging delivery through Supabase's default sender. The product creates private invitation links but does not yet send invitation email, and its roles are `owner` and `member`, not a separate `admin`. Real-account browser completion, realistic starter content, production SMTP/invitation delivery, telemetry, backup/restore, rollback rehearsal, physical devices/assistive technology, custom domain, and production promotion remain open.
