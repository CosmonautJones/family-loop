...
- No Vercel project link or deploy job exists. CI now defines an exact-head release-artifact build/upload, and a target-neutral static-envelope builder/verifier exists locally; their first hosted artifact run and every deployment remain unproven until observed.
...
| CI | Repeated green hosted PR runs through `29381271843` on `02ae226`; an exact-head single-build/digest/upload job is defined locally with no secrets or remotes | Observe the first hosted artifact job; download and independently verify its manifest/digest; then run temporary seeded-failure PRs for each blocking class and remove seeds | Green clean run; uploaded bytes verify to the job SHA/digest; lint/test/secret/migration seeds each fail for the intended reason; no secrets or remote DB access in PR jobs | Job gains write permission unnecessarily; secrets reach PR jobs; remote migration occurs; uploaded bytes mismatch; deployed bytes are rebuilt instead of promoted | **Yes:** GitHub Pro or public repo for enforced required checks; otherwise explicitly accept manual merge control |
...
No secret belongs in the runtime overlay; the Supabase publishable key is public client configuration. Envelope build and verification require the trusted artifact digest and source commit from the successful CI job/run, not the downloaded manifest alone. `connect-src` is exact to the validated backend plus WebSocket origin; `img-src` intentionally retains `https:` for attributed user media. Promotion means assigning the verified Vercel deployment/alias, not asking Vercel to rebuild different bytes.

Before upload, verify a fresh approved Vercel project uses Framework Preset `Other`, no inherited build/install command, and root (`.`) output. The envelope is a static source directory, not `.vercel/output`; stop if provider settings would rebuild it or if deployment resources contain a Function.
