...
- Manually observed: pre-migration encrypted backup/isolated restore `29516656250`, post-migration backup/restore `29517385245`, migration dry-run/apply/readback, function deploy/list readback, exact-origin preflight, and anonymous denial.
- Still unproven: verified Resend domain/sender, provider API key, click/open tracking disablement, provider acceptance/delivery, inbox receipt, Supabase Auth SMTP, new-account confirmation, and clean-browser delivered-invitation acceptance. No real email was sent, and the newer web artifact is not deployed.
