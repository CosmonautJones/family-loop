...
- Still unproven: provider acceptance/delivery, inbox receipt, Supabase Auth SMTP, new-account confirmation, and clean-browser delivered-invitation acceptance. No real email has been sent.
...
- Supabase Auth custom SMTP remains unconfigured because the dashboard handoff is still at its sign-in screen. The provider credential is available only in the secure handoff; do not recreate or paste it into chat. Delivery, inbox receipt, Auth confirmation/recovery, and separate-user acceptance remain `NOT RUN`.
