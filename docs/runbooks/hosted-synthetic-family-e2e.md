...
- The schema intentionally supports `owner` and `member`, not a separate `admin` role. The harness proves owner-versus-member restrictions; an admin role is an unimplemented product requirement, not a passed test. The current product creates a private invitation link but does not send it, so custom SMTP alone will not close family-invitation delivery.
