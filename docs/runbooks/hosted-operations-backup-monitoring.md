...
Repository search confirms the database password is consumed only as GitHub environment secret `LOOPEDIN_STAGING_DB_PASSWORD` by the encrypted-backup workflow; no Netlify production environment variable or local repository value exists. After the operator rotated the provider password and updated the GitHub secret, dispatch `29499987122` reached the dedicated staging pooler but failed password authentication during encrypted snapshot creation. Restore and upload were skipped, so no backup artifact was produced. This is a provider/GitHub secret synchronization blocker, not a repository or restore-proof regression; replace the secret through the GitHub UI and rerun before claiming post-rotation backup health.
