...
Backup workflow-dispatch run `29465195044` is GREEN. It records seven migrations, measured snapshot age/RPO 16.719 seconds and isolated restore/RTO 6.107 seconds, counts of 1 profile / 1 family / 1 membership / 4 events / 4 RSVPs / 4 messages / 3 media / 1 Auth user / 3 objects, matching object hashes/references, and outsider visibility 0/0/0. Artifact `8362712936` retains encrypted SHA prefix `9c75…`, zip SHA prefix `3d266…`, and expires 2026-08-15. Telemetry schema/configuration restored; ephemeral telemetry event and limiter rows intentionally restored as zero.

Evidence classification: the backup run was `workflow_dispatch`, not cron. Availability and hourly telemetry dispatches are hosted-observed; the first scheduled backup cron event is still unobserved. Managed PITR, cross-region/replacement cutover, destructive deletion/legal-hold apply, real SMTP/invite delivery, and production restoration remain unproven.
