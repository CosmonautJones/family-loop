...
| Remote backend | Dedicated staging `vkogznsfthirhxkqysza` matches all six migrations and passes synthetic owner/member/outsider RLS, private Storage, notifications/reminders, Realtime, and cleanup. Production and real-account proof remain open. |
...
| 013 | HOSTED ENFORCEMENT PARTIAL / OBSERVABILITY CONDITIONAL | Hosted live-invite/database/role/private-media outsider denial, hardened redaction, independent GREEN | Privacy-safe monitoring/alerts, named production ownership, retention, external assessment `NOT RUN` |
...
| 016 | STAGING COMPLETE / PRODUCTION CONDITIONAL | Exact CI artifact on separate HTTPS Netlify staging, external config/CSP/cache, configured synthetic core loop, exact-deploy rollback/restoration | Real-account completion, physical devices, custom domain, production approval/promotion `NOT RUN` |
...
| Groups/events/RSVP | Present | Four-session local creation/invite/transfer plus CRUD/RSVP/RLS pass | Hosted synthetic creation, two invitation acceptances, role/outsider denial, three events/RSVPs pass; real-account/delivery open |
...
| Backup/restore/release | Immutable-artifact/rollback, encrypted backup, isolated restore, and current-user encrypted export tooling present | Local exact-count/hash/reference/RLS recovery and scoped export downloads/decryption pass | Hosted staging release and exact-deploy rollback pass; hosted PITR/object restore/export, lifecycle policy, and deletion apply remain open |
