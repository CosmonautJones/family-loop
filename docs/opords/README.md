...
| Database definition | Nine forward migrations define the baseline, hardened private-media lifecycle, family invitation lifecycle and delivery ledger, recipient-scoped updates, private event/comment idempotency, hosted RPC grants, privacy-safe error telemetry, and account-deletion grace state. |
| Remote backend | Dedicated staging `vkogznsfthirhxkqysza` matches all nine migrations; synthetic owner/member/outsider RLS and readiness, private media, encrypted export, deletion grace, and provider-delivered invitation acceptance pass. Real owner state has three upcoming trips, one completed trip, four RSVPs/comments, and three private photos. Gmail/new-account/password completion remain open. |
...
| 006 | STAGING SYNTHETIC COMPLETE / PRODUCTION CONDITIONAL | Nine hosted migrations through `20260716150236_invitation_email_delivery`, table RLS/private Storage/protected RPC grants, owner/member/outsider matrix, local query plans | Production-cardinality plans and production deployment `NOT RUN` |
...
| 016 | STAGING COMPLETE / PRODUCTION CONDITIONAL | Release `0.1.0-2721a98ce6fc`/deploy `6a591b138c9727a4b2ca6d48` on separate HTTPS Netlify staging, external config/CSP/cache, final synthetic/readiness gates, Auth SMTP/provider-delivered invitation acceptance, newest exact-deploy rollback/restoration | Gmail/new-account/password completion, physical devices/AT, native hosted 200% observation, separate production backend, custom domain, production approval/promotion `NOT RUN` |
| 017 | HOSTED BACKUP/RESTORE + RECOVERABLE GRACE/EXPORT PASS / PURGE PARTIAL | Scheduled run `29480318427` proves initial pre-migration-nine cron execution; earlier manual `29517385245` proves all nine, and fresh manual `29527751546` at head `90250b6…` is the newest current-schema/direct-credential backup and isolated-restore proof. Lifecycle `lqa-mrni99q8-d25f64ff` and three-browser export `qa-mrniaqp3-c0cb2cea` pass exact scope, denial/recovery, integrity, outsider isolation, and zero residue. | Sustained RPO, a post-migration-nine scheduled restore, managed PITR, replacement cutover, external restore-safe journal, permanent Postgres/Storage/Auth purge, retention-orphan apply, and physical devices/AT `NOT RUN` |
...
| Backup/restore/release | Immutable-artifact/rollback, encrypted backup, isolated restore, and current-user encrypted export tooling present | Local recovery and scoped exports pass | Hosted release/rollback, scheduled encrypted backup/isolated restore, logical DB/private-object restore, and scoped export pass; managed PITR, replacement cutover, and permanent deletion apply remain open |
...
The latest staging release is source `2721a98ce6fc03a1263ebc5284d90ac936d2e571`, release `0.1.0-2721a98ce6fc`, application digest `3794214c24c33c82961fd9c96a66c9bf2fc9809e366bfb03f4ce31c138a4779a`, Netlify deploy `6a591b138c9727a4b2ca6d48`, and Supabase migration `20260716150236_invitation_email_delivery` on dedicated project `vkogznsfthirhxkqysza`. Exact rollback selected `6a58e22e48d42235e0ea40e0`, verified its release, restored `6a591b138c9727a4b2ca6d48`, and verified its release/environment without database reversal. Main CI `29518737936`, availability, telemetry, and fresh current-schema backup/restore `29527751546` at main head `90250b6d46482b3fa2b8d1a19b8eaeffc6a5148e` are GREEN.

Final synthetic core-loop run `qa-mrmw9a6b-dcce7de2` and hosted invitation-mail run `mailqa-mrnu92ro-be6d0bcc` pass with zero residue. Scheduled backup/restore `29480318427` is GREEN for initial pre-migration-nine cron execution; telemetry retention `29520494334` and availability `29521815271` are also GREEN. Fresh manual `29527751546` is the newest all-nine/current-schema restore proof. The campaign is not production-complete: sustained RPO and a post-migration-nine scheduled restore; Gmail inbox/new-account/password completion; additional approved recipient addresses; a separate production Supabase plan/org/region and SMTP boundary; physical phone/assistive-technology checks; native hosted 200% observation; managed PITR; custom domain; and explicit production approval remain gates.
