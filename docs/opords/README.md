...
| 013 | STAGING OBSERVABILITY PASS / PRODUCTION CONDITIONAL | Hosted outsider denial, no-secret availability, bounded authenticated error ingestion, 30d/24h retention, hourly aggregate monitor, and controlled alert proof | Named production ownership/routing and external assessment `NOT RUN` |
...
| 016 | STAGING COMPLETE / PRODUCTION CONDITIONAL | Release `0.1.0-08006e5e83a8` on separate HTTPS Netlify staging, external config/CSP/cache, final synthetic/readiness gates, exact-deploy rollback/restoration | Real password completion, physical devices/AT, native hosted 200% observation, custom domain, production approval/promotion `NOT RUN` |
| 017 | HOSTED LOGICAL BACKUP/RESTORE PASS / LIFECYCLE PARTIAL | Dispatch run `29465195044` restored seven migrations, telemetry config, DB/private bytes, hashes/references and owner/outsider RLS within measured RPO/RTO | First cron observation, managed PITR, replacement cutover, destructive deletion/legal-hold apply, physical devices/AT `NOT RUN` |
...
The latest staging release is source `08006e5e83a8dd85cfeb30f6fb26f8df103fa619`, release `0.1.0-08006e5e83a8`, artifact digest `e41dd1727d11888e0259a97c442f13e815243472dfcd437334c00c53b9ee0d38`, Netlify deploy `6a58428caebae3fadaf3906b`, and Supabase migration `20260716002122` on dedicated project `vkogznsfthirhxkqysza`. Rollback to `6a583390d6997d709c864c56` and restoration passed without database reversal. PR #4 and independent review were GREEN; main run `29466733884`, availability run `29466990331`, and telemetry run `29466991186` are GREEN.

Final synthetic run `qa-mrmw9a6b-dcce7de2` passed the family core loop and left zero residue. The campaign is not production-complete: real SMTP/invitation delivery and password completion, additional approved recipient addresses, a separate production Supabase plan/org/region, physical phone/assistive-technology checks, first cron observation, native hosted 200% observation, managed PITR, custom domain, and explicit production approval remain gates.
