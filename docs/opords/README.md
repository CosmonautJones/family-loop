...
| Database definition | Five forward migrations define the baseline, hardened private-media lifecycle, family invitation lifecycle, recipient-scoped update generation, and private event/comment creation idempotency. |
...
| 003 | LOCAL COMPLETE / EXTERNAL CONDITIONAL | Invite-bound signup/sign-in, session restore/logout, protected-content gate, and enumeration-safe loopback password recovery/reset with Mailpit; configured 390px recovery/offline proof | Hosted redirects, production email delivery/rate limits and operations, physical-browser autofill/deep links, and assistive technology `NOT RUN` |
...
| 006 | LOCAL COMPLETE / EXTERNAL CONDITIONAL | FK/uniqueness/RLS/direct-ID matrices, rollback-safe 20/100/100/50 production-query plans, database lint; unused-path index candidate rejected | Hosted migration state and production-cardinality plans `NOT RUN` |
...
| 008 | LOCAL COMPLETE / EXTERNAL CONDITIONAL | Exact-event Realtime subscription, authoritative Query refetch, multi-session isolation, genuine container-outage reconnect convergence, route/session teardown, and zero retained subscriptions | Hosted Realtime/RLS, physical Safari/Chrome, assistive technology, long-outage behavior, and human usability `NOT RUN` |
...
| 015 | PARTIAL/CONDITIONAL | Substantive pinned lint; three least-privilege CI jobs; local clean/five seeded failures; hosted clean run `29376063946` on `12f760d` | Hosted disposable-PR failure proof and administrator-required checks `NOT RUN` |
| 016 | PARTIAL/CONDITIONAL | Environment-neutral exact-commit artifact; strict external runtime config; one digest across local/loopback overlays; fail-closed invalid config; CSP/security/cache/SPA/mobile smoke; artifact/config rollback; hosted branch-head CI green | Named hosted environments/operators, artifact-from-hosted-CI linkage, DNS/TLS, configured staging compatibility, physical devices, deployment, and production approval `NOT RUN` |
| 017 | PARTIAL/CONDITIONAL | Encrypted local relational/private-object backup; disposable isolated restore with exact counts/hashes/references and member/outsider RLS; authenticated AES-256-GCM current-user contribution exports proven by owner/member/outsider browser downloads; bounded self-scope deletion/orphan dry runs | Hosted backup/PITR/schedule/export operations, approved RPO/RTO/retention/legal and shared-family export policy, destructive deletion/apply, physical devices/AT, and hosted restore remain `NOT RUN` |
...
|---|---|---|---|
| Session/auth gate | Present | Local Auth, invite-bound signup/sign-in, restore/logout, protected-content gate, and enumeration-safe Mailpit password recovery/reset pass | Hosted Auth redirects/mail/rate limits and physical-device/AT evidence not run |
| Groups/events/RSVP | Present | Four-session local creation/invite/transfer plus CRUD/RSVP/RLS pass | Hosted CRUD/RLS not run |
| Event chat | Durable scoped comments and exact-event subscription present | Multi-user isolation/reload plus genuine Realtime outage, resubscribe/refetch convergence, route teardown, and cleanup pass | Hosted Realtime/RLS, long-outage, physical-device/AT, and human evidence not run |
...
| Backup/restore/release | Local immutable-artifact, rollback, encrypted backup, isolated restore, and current-user encrypted export tooling present | Loopback promotion/rollback, exact-count/hash/reference/RLS recovery drill, and owner/member/outsider scoped export downloads/decryption pass | Hosted staging release/PITR/export operations/restore, approved lifecycle policy, shared-family export scope, and deletion apply not run |
