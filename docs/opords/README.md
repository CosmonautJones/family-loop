...
| 017 | HOSTED LOGICAL BACKUP/RESTORE PASS / LIFECYCLE PARTIAL | Dispatch run `29465195044` restored seven migrations, telemetry config, and Storage metadata; reconstructed private bytes matched hashes/references, and owner/outsider RLS passed. Snapshot age and drill duration were observed, not operational RPO/RTO. | First cron observation, managed PITR, working Storage rehydration/replacement cutover, hosted export, destructive deletion/legal-hold/retention-orphan apply, physical devices/AT `NOT RUN` |
