...
| 016 | PARTIAL/CONDITIONAL | Environment-neutral exact-commit artifact; strict external runtime config; one digest across local/loopback overlays; fail-closed invalid config; CSP/security/cache/SPA/mobile smoke; artifact/config rollback | Named hosted environments/operators, GitHub-hosted green commit, DNS/TLS, configured staging compatibility, physical devices, deployment, and production approval `NOT RUN` |
