...
Latest checkpoint: Dedicated hosted Supabase project `vkogznsfthirhxkqysza` records all six checked forward migrations and the first real account/family bootstrap: one confirmed Auth identity and `Travis Jones` profile, one `Jones Fam`, and exactly one owner membership with a consumed creation entitlement. Hosted Data API defaults initially left authenticated-only security-definer RPCs executable by `anon`; forward migration `20260715123221_restrict_hosted_rpc_execute_grants.sql` removes those direct grants and read-back leaves only the two intentional invitation preflight functions anonymous. Independent review then found and the fix loop closed an invited-account token-format blocker, with regression coverage for all 18 protected RPC grants. The exact fixed artifact is now published at `https://loopedin-family.netlify.app` as staging, Auth Site URL/redirect custody names exact staging/preview/future-production origins, and the hosted recovery form plus redacted Auth logs prove a recovery email was dispatched to the approved owner address. Hardened hosted synthetic run `qa-mrml3fsl-a08c4f8e` passed live-invite outsider denial and recipient acceptance, owner/member restrictions, three events, three RSVPs, two comments, reminders, seven notifications, private-media create/read/outsider-deny/delete, exact-event Realtime, and outsider database denial; the protected hash stayed unchanged, expanded automated residue was zero, and exact-ID SQL also returned zero private entitlements/operation maps. User-observed password replacement is still pending. Real state still contains one validation event named `Testing` and one RSVP, while comments/media/Storage remain empty; this residue must be replaced with realistic starter content. Roles currently support owner/member only, and family invitations do not send email. The unrelated Supabase project and personal Netlify site remain untouched. Real-account browser core loop, custom SMTP/invitation delivery, telemetry, backup/restore, staging rollback, physical devices/AT, custom domain, and production approval remain open.
