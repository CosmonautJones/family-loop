...
- Baseline/local → candidate/local → candidate/loopback → invalid/fail-closed → baseline/local rehearsal passed. No hosted/backend mutation occurred. The two overlays identify different runtime targets but use one local Supabase stack; distinct hosted backend compatibility, named environment/host/DNS/TLS owners, GitHub-hosted green commit, physical devices/AT, staging deployment, and production approval remain `NOT RUN`.
