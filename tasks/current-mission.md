...
- Post-rotation encrypted-backup dispatch `29499987122` failed safely at database password authentication; restore/upload were skipped. Dispatch `29516656250` then revalidated the credential before migration, and superseding post-migration dispatch `29517385245` is GREEN for encrypted snapshot, three private objects, all-nine hosted migration history, isolated restore, counts/hashes/references, owner/outsider RLS, and 30-day ciphertext upload. Netlify and the repository contain no database-password value.
