...
- The job retains global `contents: read`, persists no checkout credential, references no secret, provider CLI, migration, remote backend, or deploy command, and cannot promote to Vercel or Supabase.
- A dependency-free builder verifies the downloaded schema-v2 runtime artifact, rejects tampered/missing/extra/symlink entries and unsafe runtime configuration, and atomically assembles a target-neutral static Vercel envelope. A separate command re-verifies the envelope before use.
...
- First hosted artifact execution/download verification, Vercel project creation/deployment/TLS, dedicated Supabase staging, SMTP, telemetry, backup/PITR/object copy, physical-device/AT/human evidence, and production approval remain `NOT RUN`. The linked unrelated Supabase project stays quarantined.
