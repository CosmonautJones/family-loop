...
Status: `IN PROGRESS — DEDICATED STAGING OPERATIONALLY GREEN — PRODUCTION NOT READY`

Decision charter: **Correctness > Safety > Scope discipline > Speed.**

Latest checkpoint: Exact source `2721a98ce6fc03a1263ebc5284d90ac936d2e571` is release `0.1.0-2721a98ce6fc`, application digest `3794214c24c33c82961fd9c96a66c9bf2fc9809e366bfb03f4ce31c138a4779a`, and immutable Netlify deploy `6a591b138c9727a4b2ca6d48`, initially published unchanged at `https://loopedin-family.netlify.app` on `2026-07-16T18:19:21.898Z` and restored after the exact-pair rollback rehearsal at `2026-07-16T18:55:26.407Z`. Dedicated staging matches all nine reviewed migrations through `20260716150236_invitation_email_delivery`; Auth SMTP and the owner-only invitation sender are configured without exposing credentials. Hosted run `mailqa-mrnu92ro-be6d0bcc` proves owner-UI invitation creation/queueing, provider `delivered`, exact unwrapped displayed-message URL equality, separate signed-out existing-recipient acceptance across reload, session restoration, consumed-link denial/reload, exact membership-baseline restoration, and zero marked residue. Recovery provider delivery also passes, while Gmail inbox observation and password replacement do not. Earlier core-loop/private-media/Realtime/outsider, availability, telemetry, backup/restore, responsive-width, and history evidence remains valid. Fresh manual backup/isolated restore `29527751546` is GREEN at main head `90250b6d46482b3fa2b8d1a19b8eaeffc6a5148e` with all nine migrations, three private objects, expected counts/integrity, owner RLS, and outsider denial; it is the newest current-schema/direct-database-credential proof. Rollback selected prior deploy `6a58e22e48d42235e0ea40e0`, verified release `0.1.0-ce4b0c56d30b`, restored `6a591b138c9727a4b2ca6d48`, and verified release/environment headers without database reversal. Custom domain, production backend, personal site, and quarantined Supabase project remain unchanged.

Production remains blocked on production SMTP and delivered invitation/security-notice proof; the owner's private password-recovery completion; a separately provisioned production Supabase project after billing/organization/region selection; explicitly supplied additional recipient addresses; hosted external-journal custody/replay and independently reviewed destructive purge operations; retention/orphan apply; named production telemetry ownership/routing; physical iPhone Safari/Android Chrome/VoiceOver/TalkBack checks; authoritative hosted native 200% zoom; managed PITR or an accepted limitation; production custom-domain attachment; and explicit promotion approval. Hosted encrypted current-user export, hosted deletion-grace request/cancel/access denial, initial pre-migration-nine scheduled backup/isolated restore, scheduled telemetry retention, and scheduled availability are green. The leased Postgres/Storage/Auth purge, encrypted chained journal, authenticated restore gate, five crash resumes, invitation identity cleanup, and idempotent replay are green only on disposable loopback Supabase. Sustained RPO and a post-migration-nine scheduled restore remain open; the newest hosted all-nine proof is manual run `29527751546`.

OPORD 005's local response-loss gap is closed for event and comment creation. The UI retains a UUID operation key across an unchanged visible retry, rotates it after edits/success, and never auto-retries writes. Durable-local v8 and loopback Supabase private operation maps replay the same authoritative row exactly once, while distinct operations and cross-user key collisions stay distinct. The isolated family/RLS harness and retained populated-scenario verifier pass; authoritative counts remained 8/2/6/5/9/9/4/60/1. Server correlation, universal deadlines/rate enforcement, production-environment readiness, and non-event/comment write idempotency remain explicit gaps.
...
OPORD 017's authenticated export slice is complete locally and on hosted staging. The Family/no-family surfaces produce a client-encrypted current-account contribution bundle with owned photo bytes, counts, and integrity metadata. Owner, member, and outsider actual browser downloads passed decryption and cross-user exclusion without mutating the retained scenario. Shared-family archive scope, production export operations, and hosted destructive deletion remain open; the local permanent-purge boundary now has executable synthetic proof.
...
- Local export verdict: **GREEN** for the conservative current-user scope. Shared-family scope, server audit/hosted delivery, physical devices/assistive technology, and permanent account purge remain open. OPORD 017 overall remains **PARTIAL/CONDITIONAL**.
...
## 2026-07-15 — OPORD 017 account deletion grace-state checkpoint

- Added a self-service 30-day recoverable deletion request with current-password reauthentication, a five-minute database recent-sign-in guard, owner-transfer prerequisite, idempotent cancellation before the deadline, and a recovery-only authenticated shell.
- Pending accounts lose new server-authorized DB, RPC, and private-Storage access. The client resolves lifecycle status before family queries, polls every 15 seconds, and evicts protected caches plus the active family on a pending result or status failure.
- Legal-hold placement/release is service-role-only and append-only lifecycle records capture request, cancellation, and hold actions. A hold does not restore access or extend self-recovery.
- Local Supabase proof covers exact 30-day and backup-expiry boundaries, stale-auth/owner rejection, request-vs-transfer and request-vs-family-creation races, RLS/RPC/Storage denial, recovery, expired cancellation denial, outsider isolation, and exact cleanup. Database reset/lint and migration checksum validation pass.
- Independent review required and verified fixes for family-creation serialization, fail-closed cache eviction, and retaining the established one-hour signed-media lifetime until expiry-aware renewal exists. Final verdict: **GREEN, zero findings**; the executable cache test and app suite pass 86/86.
- This is a grace-state checkpoint, not erasure. Hosted staging proof, security-notice SMTP, an external restore-safe deletion journal, leased retryable Postgres/Storage/Auth purge with Auth last, creator custody/anonymization, and production adoption remain open.

## 2026-07-16 — hosted lifecycle/export delta

- Repository and dedicated staging migration history now match all eight migrations through immutable `20260716033000`; PR #7 was closed rather than weakening the historical-migration guard. Main CI `29471863503` is GREEN.
- Exact CI artifact source `a45838479634e61aedec0ceb210f79b21f58d225`, release `0.1.0-a45838479634`, and digest `aceefccf1a794f93fc145cb412db20ccf0d92c51a6a8b87549679e81065f95e6` are published as Netlify staging deploy `6a5863f6aebae3714ef3906c`.
- Superseding hosted lifecycle run `lqa-mrni99q8-d25f64ff` proves owner transfer, grace/backup boundaries, DB/RPC/Storage shutdown, service-only holds, cancellation recovery with a fresh signed byte/hash read, expired denial, outsider isolation, protected-state equality, and zero residue.
- Superseding hosted family/export run `qa-mrniaqp3-c0cb2cea` proves three actual encrypted browser downloads, exact per-account contributions across all four synthetic identities, one private-media byte/hash inclusion, outsider emptiness, wrong-passphrase/tamper denial, Realtime/core-loop counts, protected-state equality, and zero residue.
...
- Post-rotation encrypted-backup dispatch `29499987122` failed safely at database password authentication; restore/upload were skipped. Dispatch `29516656250` then revalidated the credential before migration, and earlier all-nine dispatch `29517385245` passed encrypted snapshot, isolated restore, integrity, RLS, and ciphertext upload. Fresh manual run `29527751546` at main head `90250b6d46482b3fa2b8d1a19b8eaeffc6a5148e` is the newest direct-database-credential/current-schema proof: nine migrations, three objects, expected counts/references, owner RLS 4/4/3, and outsider RLS 0/0/0 pass. Netlify and the repository contain no database-password value.
...
- Family invitation email was not implemented at this diagnosis checkpoint: the deployed UI instructed the owner to copy the link, and no trusted sender existed. The repository and staging backend now contain a reviewed sender boundary, but the deployed web UI, verified sender/provider credentials, and real delivery proof remain pending. Custom SMTP alone does not deliver the application family token.
...
- PR #9 and CI `29503514124` are GREEN at `ce4b0c56d30b`; exact artifact digest `fe7470cc…73df` is live as staging deploy `6a58e22e48d42235e0ea40e0`, with `6a5863f6aebae3714ef3906c` retained for rollback. Availability passes; clean-session invitation acceptance is still pending.
- Superseding hosted run `iqa-mrnli3aw-18956cda` proves owner-UI link creation, exact displayed copy/paste URL capture, clean-recipient preview/reload/UI acceptance, session restoration, consumed-link denial, and zero residue. Automated clipboard confirmation, new-account confirmation, revoked/expired hosted-browser observation, SMTP, and real delivered acceptance remain `NOT RUN`.
- Exact Netlify rollback to `6a5863f6aebae3714ef3906c` and restoration to `6a58e22e48d42235e0ea40e0` passed without database reversal.
- A ninth forward-only migration, pinned Edge Function, and separate owner UI action now implement the invitation sender locally. Exact owner/pending/unexpired/token checks, metadata-only storage, stable idempotency, provider-failure link preservation, stale-operation operator review, and 24-hour actor/family aggregate bounds pass focused local tests.
- Independent review was **RED** on authenticated finalizer forgery, uncounted prepared retries, and Content-Length trust. Finalization is now service-role-only with trusted-actor revalidation, every retry is cooled/counted/bounded, and a streamed 2 KiB cap precedes JSON parsing; focused and local database proofs pass.
- PR #11 merged to main at `6e8616d9e6d8a8fa7ddbebfa958aa5b11df5964c`; CI `29516031310` is GREEN and its immutable web artifact digest is `sha256:fe2b977a8b3d97250700487e885a21c87252e3e8f8f268c322e4a6991bf2b341`. Independent re-review is **GREEN** with zero findings.
- Dedicated staging now matches all nine migrations. Edge Function `send-group-invitation` version 1 is active with JWT verification, exact staging origin configuration, successful exact-origin preflight, and anonymous denial. A dedicated domain-restricted Resend sending key and verified sender are configured without exposing the credential.
- Authenticated provider readback proves `travisjohnjones.com` is verified and has no tracking subdomain; the later text-only delivery proof verifies the exact invitation URL is unwrapped. Existing DNS and the personal site were not changed.
- Main CI `29518737936` is GREEN at `2721a98ce6fc03a1263ebc5284d90ac936d2e571`; exact application digest `3794214c24c33c82961fd9c96a66c9bf2fc9809e366bfb03f4ce31c138a4779a` is immutable deploy `6a591b138c9727a4b2ca6d48`, initially published unchanged at `2026-07-16T18:19:21.898Z` and restored at `2026-07-16T18:55:26.407Z` after exact rollback to `6a58e22e48d42235e0ea40e0`. Both release headers verified and no database reversal occurred; custom domain, production backend, and personal site remain unchanged.
- Supabase Auth custom SMTP is configured through the official Management API with Resend STARTTLS, the verified Auth sender, and the domain-restricted credential. Site URL, redirect allowlist, confirmation requirement, and templates are unchanged.
- Auth accepted a recovery request to the explicitly approved owner, and Resend records the exact recovery message as `delivered` with the correct sender, recovery verification route, and canonical origin. Gmail receipt is still unproven because its connector requires reauthentication; the link was not opened and no password changed.
- Hosted run `mailqa-mrnu92ro-be6d0bcc` proves the canonical owner UI created and queued the private invitation, Resend recorded it `delivered`, its Plain Text view contained the exact unwrapped displayed URL, and the explicitly approved existing recipient accepted it in a separate signed-out session across deep-link reload. Session restoration, consumed-link denial/reload, exact membership-baseline restoration, and zero marked residue pass. The provider has no tracking subdomain and the text-only invitation contains no tracking redirect.
- Gmail inbox receipt, new-account confirmation, and the full password replacement/replay path remain `NOT RUN`; provider `delivered` is not inbox proof. No family recipient was contacted. Production SMTP remains gated on the separate production environment.
