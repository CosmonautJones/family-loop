...
Production remains blocked on production SMTP and delivered invitation/security-notice proof; the owner's private password-recovery completion; a separately provisioned production Supabase project after billing/organization/region selection; explicitly supplied additional recipient addresses; hosted encrypted current-user export proof; hosted deletion-grace proof; an external restore-safe lifecycle journal; permanent retryable Postgres/Storage/Auth purge; retention/orphan apply; named production telemetry ownership/routing; physical iPhone Safari/Android Chrome/VoiceOver/TalkBack checks; authoritative hosted native 200% zoom; observed scheduled telemetry/backup cadence; managed PITR or an accepted limitation; production custom-domain attachment; and explicit promotion approval. Local request/cancel, ownership-race denial, pending access shutdown, deadline enforcement, and legal-hold records are green but do not prove erasure.
