...
- Dedicated staging now matches all nine migrations. Edge Function `send-group-invitation` version 1 is active with JWT verification, exact staging origin configuration, successful exact-origin preflight, and anonymous denial. Provider credentials are absent and the current `ce4` web remains live, so the function is fail-closed and not user-reachable through the deployed UI.
- Verified sender/DNS, Resend tracking settings and API key, provider acceptance/delivery, inbox receipt, Auth SMTP, newer web deployment, and real new-account acceptance remain `NOT RUN`; no real email was sent.
