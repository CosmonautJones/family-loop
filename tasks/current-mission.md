...
Latest checkpoint: Dedicated hosted Supabase project `vkogznsfthirhxkqysza` records all six checked forward migrations and the first real account/family bootstrap: one confirmed Auth identity and `Travis Jones` profile, one `Jones Fam`, and exactly one owner membership with a consumed creation entitlement. Hosted Data API defaults initially left authenticated-only security-definer RPCs executable by `anon`; forward migration `20260715123221_restrict_hosted_rpc_execute_grants.sql` removes those direct grants and read-back leaves only the two intentional invitation preflight functions anonymous. Independent review then found and the fix loop closed an invited-account blocker: the client now converts the base64url route token to the hex format required by the email-match RPC, with regression coverage; the hosted grant contract also covers all 18 protected RPC signatures. The already-proven immutable Netlify deploy is now published at `https://loopedin-family.netlify.app` as staging, Auth Site URL/redirect custody names exact staging/preview/future-production origins, and the hosted recovery form plus redacted Auth logs prove a recovery email was dispatched to the approved owner address. User-observed password replacement is still pending. Remote read-back also corrected the empty-content claim: one validation event named `Testing` and one RSVP exist, while comments/media/Storage remain empty; this residue must be replaced with realistic starter content. The unrelated Supabase project and personal Netlify site remain untouched. Hosted configured-browser core-loop, custom SMTP, telemetry, backup/restore, staging rollback, physical devices/AT, custom domain, and production approval remain open.
