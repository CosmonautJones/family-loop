...
- Release build PASS from `cc0bfe5dbeb259d5bfb1fdb374c1ff12ce6008f0`: release `0.1.0-cc0bfe5dbeb2`, artifact SHA-256 `0b3fc21311ec9d86079e4bbad2e775f32c149f98fabb31a92452b3c4ab54a2ad`. Dependency audit: 11 moderate, zero high/critical; dependencies unchanged. Netlify Repository is explicitly “Not linked,” so a Git push cannot automatically deploy this candidate. Current published deploy observed September 17 is `6aa6b736b29ebed58613109d`, published September 13 via CLI, rather than the older September 10 deploy in historical notes.
...
- Hardened wrong-account handling in `1be89c1` and `830d25d`. The switch-account action now appears only after the email-bound invite RPC proves a mismatch, exposes no raw invited address, and revalidates the invite before labeling the account so a concurrently consumed, revoked, declined, or expired invite returns the generic unavailable state.
- Hardened invited signup in `af367cd` and `d154462`. Explicit duplicate-account responses and ambiguous successful signup responses without a session use honest neutral guidance; pending-confirmation guidance tells the person to confirm if a message arrives, return to the original invitation, and sign in. It does not claim delivery, and the invite token remains fragment/in-memory state rather than being put in an Auth redirect, query string, or persistent storage.
- `b6e5c66` made the mock event-loop test independent of wall-clock expiry. `44c481d` restored the high-severity dependency-audit gate while keeping Expo/React Native versions unchanged.
- Fresh app evidence at `d154462`: ESLint and TypeScript pass; 89/89 app tests pass; focused invitation privacy/adapter contracts pass. Independent security and release reviewers returned GREEN for the wrong-account revalidation and invited-signup handoff.
- Fresh disposable Supabase evidence at `d154462`: all 10 repository migrations reapplied, database lint returned zero findings, and the four-session lifecycle passed masked anonymous preview, exact-email match, wrong-account denial, correct-account acceptance/replay, consumed/revoked/declined/expired denial, unverified-email fail-closed behavior, private media, and isolation. Cleanup independently verified zero marked Auth users, groups, and Storage objects. Docker Desktop misreported the Storage container unhealthy because its exec-based probe failed; the direct Storage status endpoint returned HTTP 200 and upload/read/delete passed. An independent rerun returned GREEN.
- Fresh clean-clone verification discovered a release-only blocker: audited Metro `0.83.8` no longer exposes the internal paths/default shapes used by Expo 53's pinned CLI, so the immutable builder failed before bundling. Commit `8c643b6` keeps Expo `53.0.27`, React Native `0.79.6`, and the zero-high-severity audit result, then applies a version- and source-shape-pinned compatibility shim after clean install. The shim is idempotent, fails closed on unknown Metro input, and is invoked explicitly even when npm lifecycle scripts are disabled. Focused red/green coverage, app lint, TypeScript, 89/89 app tests, and the high-severity audit pass. The exact archived commit built release `0.1.0-8c643b6a1f13` with artifact digest `708e8cf97eeef5a405c0f36281568ddc4fb21992c19418c140d970985ebba513`.
- Independent review of that bundler correction found one separate release-gate flake: a single selector wall-clock sample measured 694.8 ms in a loaded full-suite run, then 144.9 ms in immediate isolation. Commit `4f26f8b` retains the exact 20-member/100-event/100-comment/50-media workload and unchanged 250 ms read/200 ms selector limits, but requires the median of three samples. Deterministic coverage proves one scheduling outlier is ignored and sustained slow samples still fail.
- Repeated verification then exposed the integration harness recompiling the same TypeScript graph into a fresh temporary tree for nearly every scenario; a second consecutive run took 165.5 seconds and failed one durable-reconstruction scenario while that scenario passed alone. Commit `d40cf7f` caches only the compiled module exports for the process and proves separately created services retain isolated event state. The original created-trip, RSVP, durable reconstruction, and all-view assertions remain unchanged. Three consecutive post-fix full app suites passed 91/91 in 30.9, 30.7, and 30.6 seconds; ESLint, TypeScript, and the branch diff check pass. No production application file changed in either test-gate correction.
- The earlier root-suite run in this Windows working copy was 155/156: the only failure was the previously documented `core.autocrlf=true` checksum mismatch for an unchanged historical migration. The committed migration blob is unchanged; the current expanded suite and release builder require a new `core.autocrlf=false` clean-clone run before final release-candidate certification.
- Hosted boundary: this branch has not been deployed. Current hosted and inbox evidence predates this candidate; a new-account confirmation/return/acceptance path and recipient inbox receipt on the exact candidate remain `NOT RUN`. Running them requires separate approval for a named staging target, a specifically approved disposable/new recipient and mailbox, email delivery, credentials, and cleanup. No hosted Supabase, Auth, Netlify, email-provider, recipient, or personal-site state was changed in this review.
...
- Netlify rollback proof restored prior fixed deploy `6a57f034c398b8ce7dbe3fc8` and verified `0.1.0-e52be53165a5`, then restored `6a57fc726b558b21faf57459` and verified `0.1.0-3cf45367dc85`/HTTP 200. No database reversal, Git link, custom domain, Function, Edge Function, or personal-site mutation occurred. Chrome's only console entry on the new immutable URL was the optional missing `/favicon.ico`; it is recorded as an advisory, not hidden as a clean-console pass.
- The hosted-harness independent review was initially **RED** for failure-path token redaction, missing outsider denial against a live invite, insufficient service-role group-marker revalidation, and residue wording broader than the automated checks. The fix registers every generated password/token for redaction plus a 64-hex backstop, exercises outsider match and authenticated acceptance against both live invitations, refuses cleanup unless owner/name/description all match the run marker, and checks public-profile/invitation residue explicitly.
- Hardened run `qa-mrmkwxgi-c0dca50f` was RED only when a write sent immediately after `SUBSCRIBED` missed the provider's cold-start Realtime window; exact cleanup and private-schema SQL checks were zero. A two-second post-subscription stabilization produced GREEN run `qa-mrml3fsl-a08c4f8e`: 3 members, 3 events, 3 RSVPs, 2 comments, 7 notifications, live-invite outsider denial, private-media lifecycle/denial, exact-event Realtime, unchanged protected state, and residue `{users:0, profiles:0, groups:0, invitations:0, objects:0}`. Independent SQL also returned zero exact QA entitlements and private operation-map rows while real-family counts stayed 1/1/1/1 for family/member/event/RSVP.
- The same independent reviewer re-read the hardened diff and returned **GREEN**: all four findings are closed, the two-second stabilization is bounded and truthfully scoped to later-human-write behavior, and the remaining current-scale fingerprint/private-SQL/real-account-production-device limitations are explicit.
- Hosted synthetic run 1 was RED only at Realtime cold-start observation and cleaned to zero. The scoped fix replaced the raw protocol client with the repository's installed Supabase client. Run 2 was GREEN for three members, two email-bound invitation acceptances, owner/member denial, three events, three RSVPs, two comments, seven notifications, reminder persistence, private photo upload/member read/outsider denial/authorized deletion, exact-event Realtime, and outsider database denial. The harness reported zero user/group/object residue and an unchanged protected non-QA hash; independent SQL confirmed zero QA users/profiles/groups/invitations/objects and unchanged real-family counts.
- Product truth: roles are `owner` and `member`; no `admin` role exists. Family invitations create/copy private links but do not send email. Both remain open requirements rather than inferred passes.
- Independent review was initially **RED**: invited-account signup passed the base64url route token directly to an RPC that requires 64-character hex, so real invite account creation would always fail after successful preview. A focused test first reproduced the mismatch; `signUp` now uses the existing validated `invitationTokenToHex` conversion, and the focused invite contract passes. The same fix loop expanded the hosted grant contract from a five-RPC sample to all 18 authenticated-only RPC signatures.
- Restored the already-proven immutable deploy `6a56f0408614647fe35cf968` to the separate `loopedin-family` site's HTTPS alias. Netlify recorded `published_at=2026-07-15T20:00:24.333Z`; the personal-site project, Git linkage, custom domain, Functions, and Edge Functions were not changed.
- Live Chrome loaded `https://loopedin-family.netlify.app/#/home` as `loopedin-staging · Connected` and showed only the signed-out Auth gate. The deployment still identifies exact source `5b957c51ddf127b30e6515c8b3d44fa20e46e1ce`, release `0.1.0-5b957c51ddf1`, and artifact digest `a6b6b282affa349de88ef25916577ea2c41de62da2b4c7433eec4e6a6eabb0a3`.
- A credential-safe Management API read proved hosted Auth had `site_url=http://localhost:3000`, no redirect allow-list, email confirmation enabled, and no custom SMTP. A narrow PATCH changed only Site URL and the redirect allow-list: exact staging alias, exact immutable deploy URL, and future production hostname. Read-back verified the values and preserved confirmation/signup state.
- The rendered hosted Forgot Password flow submitted the explicitly approved Travis address and showed the enumeration-safe `Check your email` state. Redacted Auth logs recorded `user_recovery_requested`, HTTP 200, and `mail.send` with `mail_type=recovery` at `2026-07-15T20:03:05Z`. Opening the message and replacing the password is manual/user-held evidence and remains pending.
- Remote read-back corrected an earlier empty-events claim: bootstrap state currently includes one validation event named `Testing` and one RSVP, with zero comments, media rows, and Storage objects. It is test residue and is not counted as realistic family starter content.
- Production SMTP, realistic starter content, full multi-user/browser/private-media/Realtime checks, telemetry, backup/restore, rollback rehearsal, physical devices/assistive technology, custom domain, and production promotion remain open.

## 2026-07-15 — Dedicated Supabase bootstrap and hosted RPC grant correction

- Netlify MCP is configured as enabled at `@netlify/mcp@1.15.1`, but this task exposed no Netlify MCP methods and the configured Windows stdio launch exited before a tool handshake (`netlify-mcp` was not recognized). A read-only Netlify CLI call scoped by exact site ID independently confirmed that `loopedin-family` still has exactly one deploy: draft `6a56f0408614647fe35cf968`, `ready`, `context=deploy-preview`, and `published_at=null`; no unrelated site inventory was requested.
- The live draft still serves release `0.1.0-5b957c51ddf1` as environment `loopedin-staging`, with `dataMode=supabase`, the exact dedicated project URL, a configured publishable client key, exact HTTP/WebSocket CSP origins, runtime `no-store`, root `no-cache`, and missing-asset `404`. The key value was not recorded. Public Auth settings confirm email-only signup with confirmation and do not expose Site URL or redirect allowlist state; the project-scoped MCP has no narrow Auth-config method, so redirect custody remains unverified and unchanged rather than being tested through a mail-sending flow or broad config push.
- Confirmed the connector and CLI link resolve only to dedicated project `vkogznsfthirhxkqysza`; the unrelated project was not queried or mutated. Remote Auth, public data, Storage objects, and migration history were empty before the authorized bootstrap.
- Applied the five checked repository migrations in their recorded order. Remote history and the private `loopedin-event-media` bucket verified afterward.
- Hosted advisors and direct `has_function_privilege` checks found current Data API defaults had granted `anon` direct execution on authenticated-only `SECURITY DEFINER` RPCs. Forward migration `20260715123221_restrict_hosted_rpc_execute_grants.sql` removes those grants while retaining the two bounded invitation preflight RPCs intended for anonymous callers.
- Passwordless Auth created exactly one identity and the profile trigger produced `Travis Jones`. The one-time entitlement and atomic group RPC then produced exactly one `Jones Fam`, one owner membership, matching creator ownership, and one consumed entitlement. A later configured validation created one event and one RSVP; comments, media rows, and Storage objects remain empty.
- Final read-back records all six migrations. Actual anonymous execution is limited to `loopedin_validate_group_invite` and `loopedin_match_group_invite_email`; the remaining advisor warnings cover those intentional bounded preflight functions, authenticated RPCs with internal authorization checks, a deliberately ungranted invitation table, and disabled leaked-password protection. Hosted configured-browser, mail custody, backup/restore, telemetry, staging rollback, physical-device/AT, and production approval remain open.
- Final gates: root PASS 114/114; migration validation PASS for six ordered/checksummed files; 218-file secret scan PASS; diff check PASS. A final CLI dry-run twice hit a hosted login-role 502 after all writes, so it is not claimed as passing; independent project-scoped migration and SQL read-back verified the complete remote state instead.
- Auth follow-up: logs prove the first one-time link confirmed the account and redirected to the configured localhost site; subsequent scanner/replay requests failed as designed. A user-approved temporary password was set without recording it in repository evidence. Hosted password login and exact `Jones Fam` owner reads passed through the public client boundary, the test session was logged out, and the stale magic-link session/refresh token was revoked. Auth Site URL/redirect custody still needs a named hosted-app decision before future email flows.
...
## 2026-07-14 — Edge and Firefox configured core-loop proof

- Source commit `31c74466e6faba2e9824cc5c7046c2fbfad5c269` rebuilt exactly as three-file release `0.1.0-31c74466e6fa`, SHA-256 `a8c41774b3cb4bbedc8cc53eb14af95a3f23227d9627e9778df58b568c0ef740`.
- Installed Edge `150.0.4078.65` through CDP and Firefox `151.0.1` through native WebDriver BiDi each passed five read-only matrices at 390/430. Signed-out, owner, member, and outsider coverage included recovery entry, session restore, notifications, exact Event Detail, private signed media, role-safe Family controls, direct-route denial, Back/deep-link/reload, five tabs/one selected tab/one main, >=48px two-dimensional controls, and zero overflow, console events, failed requests, or app/backend HTTP errors.
- Run 1 found a real release-policy defect: the validated loopback Supabase origin was in `connect-src` but absent from `img-src`, so both engines blocked a private signed family photo. The fix adds only the already validated runtime origin to `img-src`; focused coverage excludes an unrelated HTTP origin.
- Independent review ran three RED loops. Evidence now strips URL credentials/query/fragment and redacts authorization, Basic/Bearer, JWT, Supabase key, token, API-key, and password forms. Authenticated-shell and source-revision assertions fail closed, and stale precommit artifacts were superseded.
- Rendered Today, Event Detail, Memories, and Family screenshots show no engine-specific hierarchy, clipping, or control regression. Firefox's native thin scrollbar is the only visible difference. Final independent verdict: **GREEN**.
- The proof made no remote Supabase, Storage, deployment, DNS, TLS, mail, or production mutation. The retained scenario remained exactly 4 identities, 3 members, 3 trips, 6 messages, 6 RSVPs, 3 media rows, 38 notifications, 0 reminders, 3 Storage objects, and zero outsider residue.
- Safari, physical phone browsers, physical software keyboards, VoiceOver/TalkBack, and moderated older-adult use remain `NOT RUN`. Hosted deployment and dedicated hosted Supabase remain governed by `docs/runbooks/hosted-release-completion.md`.

## 2026-07-14 — OPORD 015/016 CI artifact and static deployment envelope

- TDD Run 1 was RED because the builder/verifier and CI release job did not exist. The first implementation added one exact-head build/upload job plus a dependency-free static Vercel envelope with no Function, project link, credential, provider call, dependency, or deploy command.
- KISS review extracted runtime validation and security headers from the existing preview server into one shared 59-line policy module. The builder remains responsible for independent byte-level verification and atomic no-partial-output because downloaded CI artifacts are untrusted inputs.
- TDD fix loops reproduced and closed a numeric environment-identity coercion bug and an unmanifested symbolic-link acceptance bug.
- Independent Run 1 was RED on two release blockers: the downloaded manifest authenticated its own digest without a trusted CI value, and broad hosted fallback/cache patterns would have rewritten missing extensionful assets to HTML and marked non-content-addressed future assets immutable.
- The targeted fix requires trusted CI artifact-digest and source-commit inputs in both build and verify commands; validates release ID, semantic version, epoch, unique safe sorted paths, bytes, file hashes, and canonical digest; and rejects extra/missing/symlink entries before atomic publication.
- All four CI jobs now check out the exact event SHA. `Release artifact` waits for application/security/migration success, invokes the exact-commit build once, verifies source commit, emits digest/commit outputs, and uploads through pinned `actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02` with global `contents: read` and no secrets/remotes.
- Vercel policy uses one global security-header rule with no cache collision, exact manifest-listed cache paths, immutable caching only for content-addressed static assets, `no-store` public config, and a no-cache SPA fallback that excludes reserved static directories and extensionful requests. `img-src https:` remains intentionally broad for attributed user media; `connect-src` is exact to the validated backend/WebSocket origins.
- Independent Run 2: **GREEN, zero blockers**. Four syntax checks, focused 15/15 tests, and diff check passed. Docs require trusted successful-CI outputs and a fresh approved Vercel project using Framework Preset Other, no inherited build/install command, root output, and static-only resources.
- Fresh local gate: root 112/112, app 75/75, substantive lint, TypeScript, harness, secret scan, five-migration validation, and diff check pass. Hosted artifact execution/download, provider project/settings, deployment/TLS/backend compatibility, dedicated Supabase, mail, telemetry, backup/restore, physical devices/AT/humans, and production approval remain `NOT RUN` until direct evidence exists.
- Hosted Run 1 (`29382553169`) was RED only in the newly gated `Release artifact` job after application, security/dependencies, and migration integrity passed. The exact Linux runner reproduced an unconditional Windows-only `Start-Process -WindowStyle Hidden` option in `build-web-release.ps1`; no artifact was uploaded.
- The narrow portability fix now adds `WindowStyle` only on Windows. A contract test first reproduced the failure, then the focused 15-test release/CI suite and diff check passed. A replacement hosted run remains required before artifact proof is claimed.
- Hosting direction changed after commit `4fcacc1`: the Vercel-specific envelope is superseded and will not be promoted. A separately scoped Netlify mission must adapt or replace that provider-specific slice without touching or relinking the existing personal-site Netlify project `travisjohnjones`; this fix loop changes only the universally required cross-platform artifact builder.
- Independent portability review: **GREEN, zero blockers**. Focused release/CI tests passed 15/15, PowerShell parsing and diff check passed, and timeout, process-tree termination, redirected logs, exit handling, and both 300-second bounds remain unchanged. Linux runtime proof is intentionally deferred to the replacement hosted run.

## 2026-07-14 — OPORD 016 Netlify publish-envelope replacement

- Hosting correction supersedes the Vercel provider path. The unused Vercel builder/verifier and live documentation were removed rather than retained as a second deployment implementation; historical review entries remain as provenance only. No Netlify site, account setting, provider link, deploy, Function, Edge Function, domain, or personal-site resource was accessed or changed.
- TDD Run 1 was RED: nine envelope tests failed because the Netlify builder/verifier did not exist, while the unchanged provider-free CI artifact contract remained green. The implementation then passed the focused 16-test release/CI/web suite plus three script syntax checks and diff check.
- The Netlify builder preserves trusted successful-CI artifact digest/source inputs, manifest/byte/path/symlink verification, shared runtime/CSP validation, secret-like config rejection, and atomic no-partial publication. It emits the exact artifact, canonical `runtime-config.json`, `_headers`, `_redirects`, and `deployment-envelope.json` with runtime/header/redirect digests.
- `_headers` applies the shared exact Supabase HTTP/WebSocket `connect-src`, release/environment headers, browser and Netlify-CDN `no-store` public runtime config, `no-cache` shell/manifests/policy files, and one-year immutable caching only to exact manifest-listed content-addressed `_expo/static` or `assets` filenames. Drifted headers or redirects fail verification.
- LoopedIn navigation uses URL fragments, so a deep-link reload such as `/#/event/...` requests `/` and does not need a provider rewrite. `_redirects` intentionally contains no catch-all; this preserves 404 behavior for missing scripts, JSON, images, and static resources instead of masking them with `index.html`.
- Hosted artifact proof remains run `29382987732` at exact commit `9ef55479fb4089e6b610a9b06948399b5ca59362`, internal artifact digest `f3906aeaf1620bab3fd457a360d016c6448546b04ff7a7da9ad66dcef3bbc4ec`. A new hosted run for this replacement commit is required before final handoff.
- Provider inputs still required: approved Netlify team, a new dedicated LoopedIn staging site name/domain, owner/operator and rollback authority, reviewed staging public runtime overlay, and separate authorization to create/upload. The existing personal-site project `travisjohnjones` remains explicitly quarantined.
- Independent Run 1 was RED on two release-policy gaps: the primary fragment-route request path `/` lacked an explicit browser/CDN no-cache block, and one active host-policy table still required a contradictory extensionless catch-all. A focused test reproduced the root-shell omission before the generated policy and runbook were corrected.
- Independent Run 2: **GREEN, zero blockers**. Focused tests passed 16/16, three Node syntax checks and diff check passed, and review accepted trusted identity, canonical bytes/inventory, path/symlink defenses, atomic verified rename, secret rejection, exact Supabase HTTP/WebSocket CSP, root-shell caching, drift detection, active Vercel removal, and the no-provider-mutation boundary.

## 2026-07-14 — OPORD 016 Netlify draft transport proof

- Provider inventory identifies dedicated site `loopedin-family` as `50ae6d6b-28ad-49c0-9654-c3a54899fcb5`. Draft deploy `6a56f0408614647fe35cf968` is `ready`, `context=deploy-preview`, and `published_at=null`; site fields are `published_deploy=null`, `custom_domain=null`, and `repo_url=null`. It serves only at unique URL `https://6a56f0408614647fe35cf968--loopedin-family.netlify.app`.
- The draft carries exact source `5b957c51ddf127b30e6515c8b3d44fa20e46e1ce` and CI artifact `a6b6b282affa349de88ef25916577ea2c41de62da2b4c7433eec4e6a6eabb0a3`. HTTPS/root/runtime delivery, runtime browser/CDN `no-store`, exact configured-origin CSP, manifest-verified immutable hashed asset, missing-asset 404, release/environment headers, fragment-route reload, and signed-out Chrome console/network checks passed.
- Evidence classification is **GREEN for unpublished static transport only**. It does not establish hosted Supabase migrations, authenticated backend compatibility, family/trip/comment/photo/reminder/realtime/recovery flows, mail, backup/restore, telemetry, staging rollback, physical-device/assistive-technology use, a published staging alias, production, or custom-domain behavior.
- Protected personal site `travisjohnjones` retained exact provider ID `519f3aa3-c723-4b2a-b9dd-4761e7b0a8bf`. No documentation-pass provider mutation occurred; the personal site was not linked, relinked, renamed, or deployed over.
- Read-only Supabase inventory identifies dedicated `LoopedIn` project `vkogznsfthirhxkqysza` in organization `yxedrtsnkpwfryhebswc`, `us-east-2`, `ACTIVE_HEALTHY`, created 2026-07-14; quarantined `lzscofbvecgpchokxhyb` is distinct in `us-west-2`. Email Auth is enabled, confirmation required, signup open, and phone disabled.
- All ten expected public LoopedIn table endpoints returned `404/PGRST205` in publishable-client context. This is evidence that the expected schema is not Data-API-visible, not proof of an empty database. Remote migration history remains unavailable until connector registry reload; no migration or configured data-path claim is made.
- Backup inventory reports WAL-G enabled, PITR disabled, and no usable backup-list evidence. Hosted database/object recovery remains `NOT RUN`; no key or credential value is recorded.
- Independent evidence review: **GREEN, zero blockers and zero material advisories**. Live read-only checks matched the site/deploy/personal-site fields, hosted manifest identity, root transport policy, GitHub four-job run, dedicated-versus-quarantined Supabase inventory, and backup limitations. Root tests passed 113/113 and diff check passed; no provider mutation occurred during review.

## 2026-07-15 — Real owner starter state and hosted operations

- The guarded real-owner seeder requires the exact dedicated project, approved identity/profile/family, and sole owner membership. It uses an ephemeral admin-generated magic-link session without learning or changing the owner's password, fixed idempotency keys, full attribution tuples, retry-safe private upload recovery, and redacted errors.
- Real `Jones Fam` state is now exactly three upcoming trips, one completed reunion, four owner RSVPs, four comments, and three active private photos. Each Unsplash source page/creator is persisted. A second and third run returned identical counts and SHA-256 hashes. The old validation event was removed after direct SQL inspection and exact marker/child-state checks.
- Independent seed review Run 1 RED found incomplete cascade guards, partial attribution verification, a cleanup ambiguity, and a stale test. Run 2 found an activation-response-loss deletion hazard. The final recovery path preserves or activates an existing object, aborts only a proven pending/no-object row with `body === true`, and otherwise stops for operator review. Final independent review: **GREEN**.
- Public availability passed shell/runtime/Auth/missing-asset checks for release `0.1.0-3cf45367dc85`; the scheduled workflow is no-secret and collects no behavioral data.
- The final hosted encrypted backup contained authoritative schemas, hosted migration history, and three private objects under one before/after state fingerprint. The digest-pinned, offline isolated restore matched 1/1/1/4/4/4/3/0/1/3 counts, migration/Storage inventories, three hashes, zero reference discrepancies, exact owner 4/4/3 RLS visibility, and outsider 0/0/0. Restore was 10.392 seconds from a 28.2-second-old snapshot. Three secrets are restricted to `loopedin-staging-backup`; repository copies were removed.
- The 24-hour RPO, 4-hour RTO, and 30-day retention targets are documented. Workflow schedules are configured but not yet hosted-observed because the files are not on the default branch. Managed PITR remains unavailable on the free plan; replacement-project cutover, client-error telemetry, SMTP, real multi-user recipients, physical devices, and production remain open.

## 2026-07-15 — OPORD 017 account deletion grace state

- The first independent review was **RED** on three issues: deletion could race with family creation, lifecycle-status errors retained protected Query data, and a new 60-second signed-media TTL had no renewal mechanism.
- The database fix serializes family creation and deletion on the same actor advisory lock. A live local Supabase race requires exactly one winner and proves zero pending owners; the existing ownership-transfer race remains covered.
- The client now evicts populated protected cache roots and clears the active family on either a pending status or status-query failure. The eviction boundary was extracted without changing its root set and an executable QueryClient test verifies every protected root is removed while lifecycle status remains.
- Signed media retains the established one-hour lifetime until expiry-aware renewal exists. Documentation states that already rendered/downloaded bytes and an issued URL during its remaining lifetime cannot be revoked.
- Local database reset applied all eight ordered/checksummed migrations; database lint returned zero errors. The lifecycle E2E passed recent-auth/owner/race guards, exact grace/backup boundaries, DB/RPC/Storage denial, cancellation/deadline, outsider isolation, and service-role-only legal holds. App tests pass 86/86.
- Independent re-review and the narrow extraction review were both **GREEN, zero blocker/high/medium/low findings**. Hosted staging lifecycle/browser proof, SMTP security notices, external restore-journal replay, permanent distributed purge, and production adoption remain `NOT RUN`.

## 2026-07-16 — hosted lifecycle and encrypted-export evidence

- Independent Run 1 was **RED**: export failure assertions could echo a generated passphrase or decrypted payload; ownership checks could pass vacuously and omitted one synthetic identity; Auth cleanup could miss a partially created marked user; family cleanup could orphan a private object; and cancellation overclaimed restored media access.
- The first fix added stage-only export failures, exact per-account contribution manifests, all-four-user exclusion, bounded marked-user discovery/residue verification, and a fresh post-cancel signed read whose bytes matched SHA-256. Hosted lifecycle `lqa-mrnht5tv-f69724c6` and family/export `qa-mrnhwo51-ef0be0ae` passed.
- Independent Run 2 remained **RED** because unresolved Storage deletion still allowed downstream Auth deletion to cascade away the retained media row. Both cleanup paths now stop before group/database/Auth cleanup unless object deletion is accepted and exact absence is confirmed.
- The stricter live run then exposed a provider-semantic defect in the verifier: a Supabase secret key is not an authenticated-user JWT, so the old object GET returned `400 Unauthorized` even after deletion. Cleanup failed closed and preserved the exact marked run. Operator reconciliation verified the object absent and removed only its three marked users/rows, leaving zero users/groups/requests.
- The final verifier uses the service-authorized Storage list endpoint and requires HTTP success, an array response, and no exact filename in the exact synthetic directory. Superseding lifecycle `lqa-mrni99q8-d25f64ff` and family/export `qa-mrniaqp3-c0cb2cea` are GREEN with unchanged protected fingerprints and zero residue.
- Final independent verdict: **GREEN, zero findings**. Syntax, 250-file secret scan, and diff check pass. The reviewer verified failure-output secrecy, exact export scope, all-four-user isolation, fail-closed Auth discovery, cancellation byte/hash restoration, and Storage-before-media/group/Auth cleanup.

## 2026-07-16 — hosted copied-invitation proof

- Independent Run 1 was **RED** because the harness obtained its invitation URL through an RPC rather than the owner-facing UI, and response-loss cleanup could miss a synthetic family whose create response was unavailable.
...
- Focused core/static/mocked-orchestration tests pass 12/12, and a clean local reset plus E2E prove service-only finalization, counted retries, stale/aggregate bounds, and copied-link preservation. Independent re-review returned **GREEN**, zero findings.
...
- PR #11 merged at `6e8616d9e6d8a8fa7ddbebfa958aa5b11df5964c`; main CI `29516031310` is GREEN and produced immutable artifact digest `sha256:fe2b977a8b3d97250700487e885a21c87252e3e8f8f268c322e4a6991bf2b341`.
- Pre-migration backup/restore `29516656250` revalidated the rotated credential; earlier post-migration run `29517385245` is GREEN with all-nine hosted migration history and artifact `8383282648`. `send-group-invitation` version 1 is active with JWT verification and exact-origin preflight/anonymous-denial proof.
- Authenticated Resend readback proves the existing `travisjohnjones.com` domain is verified and has no tracking subdomain. A dedicated sending-only, domain-restricted key was transferred directly into the staging Edge Function secret store; no credential value entered source, logs, evidence, or chat. The verified sender is configured without changing DNS or the personal site, and the later text-only delivery proof verifies the exact invitation URL is unwrapped.
- Main CI `29518737936` is GREEN at `2721a98ce6fc03a1263ebc5284d90ac936d2e571`. Exact application digest `3794214c24c33c82961fd9c96a66c9bf2fc9809e366bfb03f4ce31c138a4779a` was rewrapped with the unchanged staging runtime overlay and initially published unchanged as canonical staging deploy `6a591b138c9727a4b2ca6d48` at `2026-07-16T18:19:21.898Z`. Exact rollback selected `6a58e22e48d42235e0ea40e0` and verified `0.1.0-ce4b0c56d30b`; restoration selected `6a591b138c9727a4b2ca6d48` at `2026-07-16T18:55:26.407Z` and verified `0.1.0-2721a98ce6fc`/`loopedin-staging`. No database reversal occurred. Custom domain, production backend, and personal site remain unchanged.
- Custom Auth SMTP was applied through the official Management API using Resend STARTTLS on port 587 and the verified Auth sender. Readback proves the SMTP password is present while Site URL, redirect allowlist, confirmation requirement, and templates remain unchanged.
- Auth accepted one recovery request to the explicitly approved owner; Resend records the exact recovery message as `delivered` and its detail metadata contains the correct sender, recovery verification route, recovery type, and canonical LoopedIn origin. Secret values, the recovery URL/token/body, recipient, and provider identifier are excluded from repository evidence.
- Resend's domain UI shows no tracking subdomain and offers to enable tracking; the text-only delivered invitation contains the exact unwrapped canonical link and no tracking redirect. Hosted run `mailqa-mrnu92ro-be6d0bcc` proves owner-UI creation/queueing, provider `delivered`, exact displayed-message URL equality, separate signed-out existing-recipient acceptance across reload, session restoration, consumed-link denial/reload, exact real-membership baseline restoration, and zero marked residue.
- Gmail inbox receipt remains unproven because the connector requires reauthentication. The recovery link was not opened and no password changed. New-account confirmation and the full password replacement/replay path remain `NOT RUN`; provider `delivered` is not inbox proof, and no family recipient was contacted.

## 2026-07-16 — Scheduled operations and production-boundary reconciliation

- Read-only workflow evidence confirms scheduled encrypted backup/isolated restore run `29480318427` is GREEN with eight migrations before migration nine; scheduled telemetry-retention run `29520494334` and scheduled HTTPS/backend availability run `29521815271` are also GREEN. This proves initial cron execution, not sustained 24-hour RPO or a post-migration-nine scheduled restore.
- Earlier manual workflow-dispatch run `29517385245` proved all nine repository migrations. Fresh manual run `29527751546` at main head `90250b6d46482b3fa2b8d1a19b8eaeffc6a5148e` is the newest current-schema/direct-database-credential proof: backup, isolated restore, and ciphertext upload pass with nine migrations, three private objects, restored counts 1/1/1/4/4/4/3/0/1/3, zero reference drift, owner RLS 4/4/3, and outsider RLS 0/0/0. No credential value was exposed.
- Existing hosted lifecycle run `lqa-mrni99q8-d25f64ff` and family/export run `qa-mrniaqp3-c0cb2cea` prove recoverable grace/cancel/access denial and encrypted current-user exports on dedicated staging with protected-state equality and zero residue. They do not prove permanent purge, restore-journal replay, retention/orphan apply, or production erasure.
- Read-only Netlify state still has no custom domain or Git link, and `app.travisjohnjones.com` remains DNS `NXDOMAIN`. This reconciliation made no provider, DNS, personal-site, application, schema, workflow, secret, recipient, or quarantined-resource mutation.

## 2026-07-16 — local permanent account purge boundary

- Added forward migration `20260716213000_permanent_account_purge_boundary.sql`: shared group/event creators become nullable with `ON DELETE SET NULL`; private service-role-only operations lease and freeze an exact media-plus-subject-owned-Storage plan/digest, reject ineligible/held/owner accounts, require object absence before relational cleanup, and require Auth absence before a scrubbed completion tombstone.
- Added an external `.lpjournal` format with PBKDF2-SHA256/AES-256-GCM authenticated records and a ciphertext hash chain. Validation rejects plaintext/free-form fields, phase regression, truncation, corruption, wrong keys, and insufficient retention; authenticated appended-tail head recovery and post-restore reconciliation gates have focused tests.
- The operator is deliberately hard-locked to disposable loopback Supabase. It checkpoints lease/prepare, deletes real private objects, verifies exact absence, finalizes contributions, deletes Auth last, and converges after injected stops at prepare, object deletion, relational finalization, and Auth deletion.
- Initial E2E diagnostics found cleanup masking and then a local Storage semantic mismatch: an absent authenticated object returned HTTP `400` with exact `404/not_found` JSON instead of HTTP `404`. Cleanup now removes object/media state in dependency order, and the operator accepts only those two explicit absence forms while failing closed on all others.
- Focused contract tests pass 5/5, journal tests pass 5/5, and the full local Supabase purge E2E passes all four crash/resume cases with real Storage deletion, neutral preserved shared rows, outsider denial, no plaintext subject/path in the journal, and idempotent replay. Hosted apply, external provider custody, restore-time hosted reconciliation, production deletion, and independent final review remain `NOT RUN`.
- Final local gates: all ten migrations reset successfully; database lint returned zero findings; the destructive E2E reran GREEN; focused purge/journal tests pass 10/10; app lint, TypeScript, and 87/87 app tests pass; the harness and 292-file secret scan pass. The root suite is 152/153 locally: its only failure is the pre-existing Windows working-copy line-ending mismatch for three unchanged historical migration blobs, whose committed hashes still equal their recorded checksums. No historical migration was rewritten to conceal that platform-local result.

### Independent G3 Fix Wave 1

- G3 requested four corrections on `980d4b9`: completed operations were hidden behind deleted-request eligibility, inbound invitation identity associations were incomplete, arbitrary caller reconciliation IDs could open restore traffic, and missing/current journal heads could be recovered without a strict authenticated anchor extension.
- The lease now discovers its exact completed tombstone first. A fifth crash seam commits database completion before the external checkpoint; retry verifies the journal digest and Auth absence, appends only the missing completion record, and converges.
...
- Fresh root suite 159/159, app suite 91/91, substantive lint, TypeScript, harness, 300-file secret scan, and diff checks pass. Build and browser candidate evidence are recorded separately after execution.
- Read-only hosted availability and signed-out browser pass on older release `0.1.0-eca9bb8f7be8`. GitHub availability and backup jobs fail before execution because of the account payment/spending-limit restriction. GO-LIVE now distinguishes current observations from the July handoff's superseded readiness claims.
- No remote push, PR, deployment, hosted data change, email, password, billing change, or physical-device claim occurred. Docker integration is not run because the local engine is stopped. New-account hosted invitation confirmation remains unproven; its retained harness is obsolete.

## 2026-09-10 — exact-candidate invitation harness

- Removed the stale fixed release identifier. The PowerShell wrapper requires a trusted local artifact, full source SHA, artifact digest, and non-sensitive authorization reference. Public preflight checks root HTML and all manifest payload bytes, exact manifest identity, bounded runtime configuration, fixed staging backend, and runtime cache policy before credential retrieval. `-PreflightOnly` performs no credential lookup or fixture mutation.
- Updated current account navigation and restored-session selectors. Added synthetic wrong-account recovery and consumed/revoked/expired denial paths. Expiry updates require exactly one pending invitation under the marked QA owner/family; cleanup is skipped when preflight fails before any fixture attempt. Updated hosted browser scenarios and cleanup are NOT RUN.
- Independent review identified three defects: root HTML was not hashed, a second runtime read was unbounded, and the restored-session assertion still expected the obsolete visible Family tab. All were corrected. Two regression tests reproduced failures before fixes. Reviewer reran focused tests 10/10 and closed all findings.
- Final root suite: 169/169. JavaScript and PowerShell syntax, 302-file secret scan, and diff checks pass. Prior app 91/91, lint, TypeScript, migration, build, and browser evidence remain applicable to unchanged application code.
- Read-only preflight against application artifact source c82b33411c7d3effa2faef7a55c9bb646e798d4d refused the older live release before credentials. This is guard evidence, not a hosted scenario pass. No remote push, deployment, shared data, recipient, password, or billing changes occurred. Confirmation-required signup and inbox receipt remain explicitly unverified.

## 2026-09-10 — local and hosted workflow linting
...
## 2026-09-10 — verified publication to existing LoopedIn site

- After Travis logged into Netlify to continue the prepared publication, uploaded the verified ZIP through the existing loopedin-family project. Netlify confirms published immutable deploy 6aa2a671aa89755f10dca32c. The former published deploy 6a5a3b8a7680daa8bfdd67bf remains available as the recorded rollback target; no rollback or database change was performed.
