...
- Pre-migration backup/restore `29516656250` revalidated the rotated credential; superseding post-migration run `29517385245` is GREEN with all-nine hosted migration history and artifact `8383282648`. `send-group-invitation` version 1 is active with JWT verification and exact-origin preflight/anonymous-denial proof. Only the application origin is configured; provider key/sender/tracking, Auth SMTP, web deployment, real email, and delivered acceptance remain unproven.
