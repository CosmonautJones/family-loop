...
- The root suite in this Windows working copy is 155/156: the only failure is the previously documented `core.autocrlf=true` checksum mismatch for an unchanged historical migration. The committed migration blob is unchanged; a `core.autocrlf=false` clean-clone run is required before final release-candidate certification.
